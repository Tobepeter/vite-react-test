...
export const ZustandShallow = () => {
  ...
  // 老的版本，支持在 useStore 传递一个 shallow 的 middleware, v5 似乎不支持了
  // const { count1, count2 } = useStore(useShallow(state => ({ count1: state.count1, count2: state.count2 })))

  // - old
  // const { count1, count2 } = useStore(state => ({ count1: state.count1, count2: state.count2 }))
  ...
  return (
    <div>
      ...
      <Button onClick={() => changeCount1(count1 + 1)}>changeCount1</Button>
      <Button onClick={() => changeCount2(count2 + 1)}>changeCount2</Button>
    </div>
  )
}
